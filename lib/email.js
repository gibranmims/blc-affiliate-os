...
<body>
  <div class="container">
    ...
    <div class="footer">The Bikini Line Co. · This email was sent because you entered the Win Your Money Back Challenge.</div>
  </div>
</body>
</html>`;
}
...
// ── Team: Pro Partner Outreach Follow-up Digest ──────────────────

async function sendPartnerFollowupDigest(leads) {
  const rows = leads.map(l => `
    <div class="detail-row"><span class="detail-label">${l.esthetician_name || l.ig_handle}</span>
    <span class="detail-value">@${l.ig_handle} · ${l.studio_name || '—'} · ${l.city || '—'}</span></div>
  `).join('');
  const html = wrap(`
    <h1>${leads.length} Pro Partner follow-up${leads.length === 1 ? '' : 's'} due today</h1>
    <p>These leads were DM'd and haven't replied — time for a follow-up message.</p>
    <div class="divider"></div>
    ${rows}
    <div class="divider"></div>
    <a href="${APP_URL}/?page=partner-outreach" class="btn">Open Pro Partner Outreach</a>
  `);
  return getResend().emails.send({
    from: FROM,
    to: TEAM_EMAIL,
    subject: `${leads.length} Pro Partner follow-up${leads.length === 1 ? '' : 's'} due today`,
    html
  });
}
